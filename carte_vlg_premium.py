...
Dark theme · DBSCAN clusters · Heatmap · Filtres par année · Stats dashboard
"""
import pandas as pd
import numpy as np
import folium
...
db = DBSCAN(eps=100/6_371_000, min_samples=3, algorithm="ball_tree", metric="haversine")
data["cluster"] = db.fit_predict(coords)
n_clusters = int((data["cluster"] >= 0).sum() > 0 and data[data["cluster"] >= 0]["cluster"].nunique())
n_noise = int((data["cluster"] == -1).sum())
print(f"Clusters: {n_clusters}, Isolés: {n_noise}")
...
# ── 6. Heatmap layer ─────────────────────────────────────────────────────────
heat_data = data[["latitude", "longitude", "valeur_fonciere"]].copy()
heat_data["w"] = (heat_data["valeur_fonciere"] - heat_data["valeur_fonciere"].min()) / (
    heat_data["valeur_fonciere"].max() - heat_data["valeur_fonciere"].min() + 1
)
heatmap_fg = folium.FeatureGroup(name="🌡️ Heatmap (intensité prix)", show=False)
HeatMap(
...
poly_fg = folium.FeatureGroup(name="🗺️ Zones DBSCAN", show=True)
for cid in sorted(data[data["cluster"] >= 0]["cluster"].unique()):
  ...
  <div id="vlg-body">
    <div class="kpi-grid">
      ...
      <div class="kpi">
        ...
        <div class="lbl">Zones DBSCAN</div>
      </div>
      <div class="kpi">
    ...
    <div style="margin-top:14px;padding-top:10px;border-top:1px solid rgba(255,255,255,.08);
      ...
      Source : DVF · data.gouv.fr · DBSCAN ε=100m min=3pts
    </div>
  </div>
</div>
"""
...
# ── 12. Save ──────────────────────────────────────────────────────────────────
out = "/home/user/carte_vlg_premium.html"
m.save(out)

# Inject custom CSS into the saved file for popup polish
